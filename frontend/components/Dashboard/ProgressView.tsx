...
const ProgressView: React.FC<ProgressViewProps> = ({ userId }) => {
  ...
  return (
    <div className="animate-reveal max-w-7xl mx-auto pb-20">
      <div className="mb-16">
        <h1 className="text-5xl font-extrabold text-primary dark:text-white tracking-tight mb-3">My Progress</h1>
        <p className="text-slate-400 dark:text-slate-500 font-medium text-lg">See how much you have learned in {courses.length} courses.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-12 rounded-[3rem] shadow-sm">
            <h3 className="text-2xl font-extrabold text-primary dark:text-white mb-12 flex items-center gap-4">
              <Activity className="text-secondary" /> Current Courses
            </h3>
              ...
              {courses.length > 0 ? courses.map((course, i) => (
                <div key={course.id}>
                  <div className="flex justify-between items-end mb-4">
                    ...
                    <span className="text-sm font-black text-secondary uppercase tracking-[0.2em]">{course.level}</span>
                  </div>
                  <div className="w-full bg-slate-50 dark:bg-slate-800 rounded-full h-3">
                    <div className={`h-3 rounded-full bg-indigo-500 transition-all duration-1000 shadow-md`} style={{ width: `${(course as any).progress || 0}%` }} /> 
                  </div>
                </div>
              )) : (
                <p className="text-slate-400">No courses yet. Start learning!</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-12 rounded-[3rem] shadow-sm">
            <h3 className="text-2xl font-extrabold text-primary dark:text-white mb-12 flex items-center gap-4">
              <Award className="text-secondary" /> My Badges
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              {[
                { title: 'Verified User', org: 'Admin', date: 'Active', status: 'Verified' },
                { title: 'First User', org: 'CareerGuide', date: '2024', status: 'Badge' },
              ].map((c, i) => (
                <div key={i} className="bg-slate-50 dark:bg-slate-800 p-8 rounded-3xl flex items-center gap-6 group hover:bg-secondary hover:text-white transition-all shadow-sm border border-slate-100 dark:border-slate-700 hover:border-secondary">
                  <div className="w-16 h-16 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center text-secondary shadow-md shrink-0 group-hover:scale-110 transition-transform">
        ...
        <div className="space-y-12">
          {/* Points/Level System */}
          <div className="bg-primary dark:bg-slate-900 text-white p-12 rounded-[3rem] text-center shadow-2xl relative overflow-hidden group border border-transparent dark:border-slate-800">
            ...
            <div className="w-28 h-28 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-10 border border-white/20 shadow-xl group-hover:scale-110 transition-transform">
              <Star className="text-secondary" size={48} fill="currentColor" />
            </div>
            ...
            <div className="w-full bg-white/5 rounded-full h-2.5 mb-3">
              <div className="bg-secondary h-2.5 rounded-full shadow-[0_0_15px_rgba(37,99,235,0.6)]" style={{ width: `${progressPercent}%` }} />
            </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-12 rounded-[3rem] shadow-sm">
            <h3 className="text-2xl font-extrabold text-primary dark:text-white mb-10 flex items-center gap-4">
              <Target className="text-secondary" /> Weekly Sprint
            </h3>
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex items-center gap-5 p-3 hover:bg-white dark:hover:bg-slate-800 rounded-2xl transition-all cursor-pointer">
                  <div className={`w-7 h-7 rounded-lg border-2 transition-all flex items-center justify-center shrink-0 ${g.done ? 'bg-secondary border-secondary' : 'border-slate-200 dark:border-slate-700'}`}>
