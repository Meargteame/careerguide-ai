...
const RoadmapGenerator: React.FC<RoadmapGeneratorProps> = ({ onCourseCreated, userId = 'mock-user-id', initialQuery }) => {
  ...
  React.useEffect(() => {
    if (initialQuery && !result && !loading && !hasAutoGenerated.current) {
        ...
        triggerGeneration(`Goal: ${initialQuery}, Level: Beginner, Time: 10 hours/week`);
    }
  }, [initialQuery]);
  ...
  const handleGenerate = () => {
    const query = `Goal: ${input}, Experience Level: ${experienceLevel}, Time Commitment: ${timeCommitment}`;
             ...
             <div className="mt-24 w-full text-left animate-in slide-in-from-bottom-5 duration-500">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {history.map(item => (
                    <div 
                      key={item.id} 
                      onClick={() => { setResult(item.road_data); setCurrentRoadmapId(item.id); setViewMode('roadmap'); }}
