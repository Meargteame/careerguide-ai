    ...
    return (
      <div className="fixed inset-0 z-[200] bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
        <div className="bg-white dark:bg-slate-900 p-12 rounded-[3rem] shadow-2xl max-w-md w-full text-center border border-slate-100 dark:border-slate-800">
          <Loader2 className="animate-spin text-secondary mx-auto mb-6" size={48} />
          <h3 className="text-3xl font-black text-primary dark:text-white mb-2">Making Quiz...</h3>
    ...
    return (
      <div className="fixed inset-0 z-[200] bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
        <div className="bg-white dark:bg-slate-900 p-12 rounded-[3rem] shadow-2xl max-w-md w-full text-center border border-slate-100 dark:border-slate-800">
          ...
          <h3 className="text-3xl font-black text-primary dark:text-white mb-4">Quiz Error</h3>
          ...
          <div className="flex gap-4 justify-center">
             ...
             <button onClick={() => window.location.reload()} className="px-10 py-4 bg-secondary text-white rounded-2xl font-bold hover:opacity-90 transition-all shadow-lg shadow-secondary/20">Retry</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="fixed inset-0 z-[200] bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-6 animate-reveal">
        <div className="bg-white dark:bg-slate-900 p-16 rounded-[3rem] shadow-2xl max-w-md w-full text-center border border-slate-100 dark:border-slate-800 relative overflow-hidden">
          ...
          <h3 className="text-4xl font-black text-primary dark:text-white mb-2">You Finished!</h3>
          <p className="text-slate-400 uppercase tracking-widest text-xs font-bold mb-8">Score</p>
          
          <div className="mb-10">
            <span className="text-7xl font-black text-primary dark:text-white">{score}</span>
            <span className="text-3xl text-slate-300 font-bold">/{questions.length}</span>
          </div>
          ...
          <button onClick={onClose} className="w-full bg-primary dark:bg-secondary text-white py-5 rounded-2xl font-bold uppercase tracking-widest text-sm hover:opacity-90 transition-all shadow-xl active:scale-95 shadow-secondary/20">
            Back to Dashboard
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[200] bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-6 animate-in zoom-in-95 duration-300">
      <div className="w-full max-w-3xl bg-white dark:bg-slate-900 rounded-[3rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col max-h-[90vh]">
        ...
        <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
          <div>
            <h4 className="text-xs font-black uppercase tracking-widest text-secondary mb-1">Quiz</h4>
            <h3 className="text-xl font-bold text-primary dark:text-white truncate max-w-md">{topic}</h3>
          </div>
          <div className="flex items-center gap-4">
             <div className="text-right">
                <span className="block text-2xl font-black text-primary dark:text-white leading-none">0{currentQuestion + 1}</span>
        ...
        <div className="p-10 md:p-14 overflow-y-auto flex-1">
          <h2 className="text-2xl md:text-4xl font-extrabold text-primary dark:text-white mb-12 leading-tight">
            {q.question}
          </h2>
          ...
          <div className="space-y-4">
            {q.options.map((option, idx) => {
              ...
              return (
                ...
                >
                  <span className={`text-lg font-bold ${isCorrect || isWrong ? '' : 'text-primary dark:text-slate-300'}`}>{option}</span>
          ...
          {showResult && (
            <div className="mt-10 animate-reveal">
               <div className="p-6 bg-slate-50 dark:bg-slate-900 rounded-2xl border-l-4 border-secondary">
                 <h5 className="text-xs font-black uppercase tracking-widest text-secondary mb-2">Explanation</h5>
                 <p className="text-slate-600 dark:text-slate-400 font-medium leading-relaxed">{q.explanation}</p>
               </div>
               
               <div className="mt-8 flex justify-end">
                 <button 
                   onClick={handleNext}
                   className="flex items-center gap-3 bg-primary dark:bg-white text-white dark:text-primary px-10 py-4 rounded-xl font-bold uppercase tracking-widest text-xs hover:opacity-90 transition-all shadow-xl active:scale-95"
                 >
